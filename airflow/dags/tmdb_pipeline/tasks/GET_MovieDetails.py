...
ENDPOINT_TEMPLATE = "/movie/{movie_id}"
BATCH_SIZE = 500
THROTTLE_SECONDS = 0.03  # ~33 req/s, safely below TMDB's ~40 req/s limit
LOG_INTERVAL = 1000


def _fetch_details(session: requests.Session, movie_id: int, headers: dict) -> dict | None:
...
def _insert_batch(cur, batch: list[tuple]) -> None:
    insert_sql = """
        INSERT INTO TMDB_MOVIE_DETAILS_RAW (movie_id, endpoint, raw_json)
...
def ingest_movie_details() -> None:
    ...
    try:
        ...
        cur.execute("""
            CREATE TABLE IF NOT EXISTS TMDB_MOVIE_DETAILS_RAW (
        ...
        cur.execute("""
            ...
            FROM TMDB_MOVIE_RAW
            WHERE raw_json:popularity::FLOAT >= 1
              AND movie_id NOT IN (SELECT DISTINCT movie_id FROM TMDB_MOVIE_DETAILS_RAW)
            ORDER BY movie_id
        """)
        movie_ids = [row[0] for row in cur.fetchall()]
        total = len(movie_ids)
        print(f"[INFO] Details: {total} movies to process")

        if not movie_ids:
            print("[INFO] No new movies to fetch details for.")
            return
        ...
        for movie_id in movie_ids:
            data = _fetch_details(session, movie_id, headers)
            ...
            if processed % LOG_INTERVAL == 0:
                print(f"[INFO] Details: processed {processed}/{total}")

        if batch:
            _insert_batch(cur, batch)
            conn.commit()

        print(f"[INFO] Details: done. Inserted {processed} records into BRONZE.TMDB_MOVIE_DETAILS_RAW")
    finally:
        cur.close()
        conn.close()
