...
with source as (
    select * from {{ ref('base_bronze_TMDB_MOVIE_DETAILS_RAW') }}
),
deduplicated as (
    select
        RAW_JSON:id::INTEGER                          as MOVIE_ID,
        NULLIF(RAW_JSON:imdb_id::VARCHAR, '')         as IMDB_ID,
        RAW_JSON:title::VARCHAR                       as TITLE,
        NULLIF(RAW_JSON:tagline::VARCHAR, '')         as TAGLINE,
        RAW_JSON:status::VARCHAR                      as STATUS,
        RAW_JSON:runtime::INTEGER                     as RUNTIME,
        RAW_JSON:budget::INTEGER                      as BUDGET,
        RAW_JSON:revenue::INTEGER                     as REVENUE,
